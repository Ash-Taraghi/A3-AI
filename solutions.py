...
def normalize(self):
    ...
    "*** YOUR CODE HERE ***"
    total = self.total()
    #step 2. In the case where the total values is not 0
    # we want to divide all keys by total so they sum to 1
    if total != 0:
        for i in self.keys():
            self[i] /= total
...
def sample(self):
    ...
    keys = list(self.keys())
    total = self.total()
    # rand represents a randomized total value
    rand = random.random() * total
    sum = 0
    slot = 0
    for value in self.values():
        # grab sum of all values of all keys in dict
        sum = sum + value
        if sum > rand:
            return keys[slot]
        else:
